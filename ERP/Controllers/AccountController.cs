...
namespace ERP.Controllers
{
    ...
    public class AccountController : SettingsController
    {
        ...
        public AccountController(IRoleAccessStore roleAccessStore, UserManager<UserMasterMetadata> userManager, SignInManager<UserMasterMetadata> signInManager)
        {
        ...
        [HttpGet]
        [AllowAnonymous]
        public async Task<IActionResult> Home(string returnUrl = null)
        {

           // await _roleAccessStore.RemoveRoleAccessAsync(USERID.ToString());
        ...
        public async Task<IActionResult> Login(LoginViewModel model, string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                   ...
                   var item = _userManager.FindByEmailAsync(model.Email).Result;
                    System.Diagnostics.Debug.WriteLine("==RoleName==>" + item.RoleName);
                    System.Diagnostics.Debug.WriteLine("==model==>" + model);
                    bool isActive = item.IsActive;
                    if (isActive && item.RoleName.Equals("Developer"))
                    {
                        return this.RedirectToAction("Index", "Home", new { area = "Developer" });
                    }
                    else if (isActive && item.RoleName == "SuperAdmin")
                    {
                        return this.RedirectToAction("Index", "Home", new { area = "SuperAdmin" });
                    }
                    else if (isActive && item.RoleName == "Admin")
                    {
                        return this.RedirectToAction("Index", "Home", new { area = "Admin" });
                    }
                    else if (isActive && item.RoleName == "HR Super Admin")
                    {
                        return this.RedirectToAction("Index", "Home", new { area = "HR Super Admin" });
                    }
                    else if (isActive && item.RoleName == "ERP User")
                    {
                        return this.RedirectToAction("Index", "Home", new { area = "ERP User" });
                    }
